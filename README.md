...
在 Linux amd64 机器上可以直接构建并运行容器版本：

```bash
# 1. 构建镜像
docker build -t smartlink .

# 2. 运行容器（默认监听 0.0.0.0:5000）
docker run -d --name smartlink -p 5000:5000 \
  -e SMARTLINK_HOST=0.0.0.0 \
  -e SMARTLINK_PORT=5000 \
  smartlink
```

> 参数说明：
> - 镜像启动时默认附带 `--no-browser --no-tray`，适用于无图形的容器环境。
> - 可以通过环境变量 `SMARTLINK_HOST`、`SMARTLINK_PORT` 控制监听地址/端口，或在启动命令中传递 `--host/--port` 覆盖。

启动完成后访问 `http://<服务器IP>:5000` 即可打开 Web 控制面板。
